...
class Config_Test:
    ...
    def create_input_path(self, create_dataset_path):
        ...
        if self.use_LHD:
            sub_path = 'LHD/c%.1f/fold_%d_%.1f' % (self.threshold, self.fold, self.threshold)
        else:
            sub_path = 'HMDARG-DB/fold_%d'%self.fold

        if create_dataset_path:
            root_dir = 'inputs'
            return os.path.join(root_dir, '%s.test2.csv' % sub_path)
        else:
...
def evaluate_by_len(model_generator, input_encoder, config,  df, start_seq_len = 512, start_batch_size = 32, increase_factor = 2):#output_spec,
    ...
    for len_matching_dataset, seq_len, batch_size, index in split_dataset_by_len(df, start_seq_len = start_seq_len, start_batch_size = start_batch_size, \
        ...
        # Extract the last 512 dimensions
        y_pred = y_pred[:, -512:]
    ...
    for i in list(range(len(df))):
        index_to_label[index_list[i]] = y_pred[i]
...
def calculate_attentions(model, input_encoder, seq, seq_len = None):
    ...
    print(f"X_seq_length:{X_seq.shape}")
    ...
    print(f"model_inputs_length:{len(model_inputs)}")
    print(f"model_inputs[0].shape:{model_inputs[0].shape}")
    print(f"model_inputs[1].shape:{model_inputs[1].shape}")
    print(f"model_attentions_length:{len(model_attentions)}")
...
def main(config):
    ...
    df = evaluate_by_len(finetuned_model, input_encoder, config, test_set, start_seq_len = 512, start_batch_size = 32)
    ...
    if config.get_all_attention:
        ...
        for i in test_set.index:
            ...
            keras_finetuned_model = finetuned_model.create_model(seq_len)
            finetuned_attention_values, finetuned_seq_tokens, finetuned_attention_labels = calculate_attentions(keras_finetuned_model, input_encoder, seq,\
                    seq_len = seq_len)
