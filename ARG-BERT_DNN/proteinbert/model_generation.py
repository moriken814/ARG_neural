...
class ModelGenerator:
    ...
    def _init_weights(self, model):
        ...
        if self.optimizer_weights is not None:
            expected_weights_count = len(model.optimizer.get_weights())
            actual_weights_count = len(self.optimizer_weights)

            # Print the number of weights
            log(f"Expected number of optimizer weights: {expected_weights_count}")
            log(f"Actual number of optimizer weights: {actual_weights_count}")
...
class FinetuningModelGenerator(ModelGenerator):
    ...
    def create_model(self, seq_len, freeze_pretrained_layers = False):
        ...
        # output_layer = keras.layers.Dense(7, activation = 'softmax')(last_hidden_layer)   # negative + 6 class mechanism = 7
        loss = 'sparse_categorical_crossentropy'      
        model = keras.models.Model(inputs = model_inputs, outputs = last_hidden_layer)
...
class InputEncoder:
    ...
    def encode_X(self, seqs, seq_len):   # seq_len = 512 or 1024
        return [
